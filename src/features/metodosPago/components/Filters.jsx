...
import { clear_filter_store } from '../../../store/metodospagosStore/metodospagosStore';
import { set_filter_store_thunk, get_filter_thunk } from '../../../store/metodospagosStore/metodosThunks';
import {
...
export default function MetodosPagoFilters({ resultCount }) {
  const dispatch = useDispatch();
  const { filter_search, filter_status, filter_environment, filter_deleted } = useSelector((s) => s.metodospagosStore);
  ...
  const activeCount = [filter_search, filter_status, filter_environment]
    ...
    >
      ...
      <Collapse in={expanded}>
        ...
        <Box sx={{ px: 2.5, pt: 2.5, pb: 2 }}>
          ...
          >
            {/* Busqueda — ocupa 2 columnas en md */}
            <Box sx={{ gridColumn: { xs: '1', sm: '1 / -1', md: '1 / 3' } }}>
              <TextField
                fullWidth
                name="filter_search"
                label="Buscar metodo de pago"
            ...
            <TextField
              select fullWidth
              name="filter_environment"
              label="Entorno"
              value={filter_environment}
              onChange={e => handleFilterChange('filter_environment', e.target.value)}
            >
              <MenuItem value="all"><em>Todos los entornos</em></MenuItem>
