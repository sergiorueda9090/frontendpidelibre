...
import { clear_filter_store } from '../../../store/attributeStore/attributeStore';
import { set_filter_store_thunk, get_filter_thunk } from '../../../store/attributeStore/attributeThunks';
import {
...
export default function AttributesFilters({ resultCount }) {
  const dispatch = useDispatch();
  const { filter_search, filter_deleted } = useSelector((s) => s.attributeStore);
  ...
  const activeCount = [filter_search]
    .filter(v => v && v !== '').length + (filter_deleted ? 1 : 0);
    ...
    >
      ...
      <Collapse in={expanded}>
        ...
        <Box sx={{ px: 2.5, pt: 2.5, pb: 2 }}>
          ...
          >
            {/* Búsqueda — ocupa 3 columnas en md */}
            <Box sx={{ gridColumn: { xs: '1', sm: '1 / -1', md: '1 / 4' } }}>
              <TextField
                fullWidth
                name="filter_search"
