...
import { clear_filter_store } from '../../../store/attributeValuesStore/attributeValuesStore';
import { set_filter_store_thunk, get_filter_thunk } from '../../../store/attributeValuesStore/attributeValuesThunks';
import {
...
export default function AttributesValuesFilters({ resultCount }) {
  const dispatch = useDispatch();
  const { filter_search, filter_attribute, filter_deleted } = useSelector((s) => s.attributeValuesStore);
  ...
  const activeCount = [filter_search]
    .filter(v => v && v !== '').length
    + (filter_attribute ? 1 : 0)
    + (filter_deleted   ? 1 : 0);
          ...
          >
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: '1 / -1', md: '1 / 3' } }}>
              <TextField
                ...
                name="filter_search"
                label="Buscar valor"
                placeholder="Valor o nombre de atributo..."
                value={filter_search}
            ...
            {/* Filtro por atributo padre */}
            <TextField
              select fullWidth
              label="Atributo"
              value={filter_attribute ?? ''}
              onChange={e => handleFilterChange('filter_attribute', e.target.value || null)}
            >
              <MenuItem value=""><em>Todos los atributos</em></MenuItem>
              {attributes.map(a => (
